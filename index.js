...
const pyra_line_geo = new LineSegmentsGeometry().fromEdgesGeometry(pyra_edges_geo);
...
eye_outl.position.set(0, 0, .1)

const eye_outl2 = eye_outl.clone()
eye_outl2.position.set(0, -.005, 0.1)

const eye_outl3 = eye_outl.clone()
eye_outl3.position.set(0, .005, 0.1)
...
pupil_ellipse.ellipse(0, 0, .015, .04, 0, 2*Math.PI, false, 0);
...
const brim_line_geo = new LineSegmentsGeometry().fromEdgesGeometry(brim_edges_geo);
...
const top_line_geo = new LineSegmentsGeometry().fromEdgesGeometry(top_edges_geo);
...
light_left.position.set(-.5, .75, 2.2);
// Left target
const left_target = new THREE.Object3D();
left_target.position.set(.25,.25,.25);
...
light_right.position.set(.5, .75, 2.2);
// Right target
const right_target = new THREE.Object3D();
right_target.position.set(-.25,.25,.25);
...
glyph_group.position.set(0,.5,-1.3);

scene.add(floor, floor2, bill, glyph_group, light_left, left_target, light_right, right_target);
...
function animate() {
    ...
    let bill_z_s = 0.2*Math.sin(time*1.5)
    let bill_x_s = 0.08*Math.sin(time*1.5);
    let bill_y_s = 0.1*Math.cos(time*1.5);
    let bill_rz_s = 0.01*Math.cos(time*1.5);

    controls.update();
    // renderer.render(scene, camera)
    composer.render() // render while keeping effects
    floor.position.z = (time * .15)%2;
    floor2.position.z = (time * .15)%2 - 2;
    ...
    glyph_group.position.z = bill_z_s -1;
